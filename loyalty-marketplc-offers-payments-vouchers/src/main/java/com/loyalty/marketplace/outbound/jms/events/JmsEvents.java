...
public class JmsEvents {
    ...
    public void publishReplyEvents(Object event, String queueName, String correlationId){
    	LOG.info(
				"inside publishReplyEvents method of class EventHandler publising event {} to queue {} correlationId={}",
				event, queueName, correlationId);

		boolean flag = true;
		try {
			this.jmsTemplate.setPubSubDomain(false);
			this.jmsTemplate.convertAndSend(queueName, event, createMessage(correlationId, null, false));

		} catch (Exception e) {
			LOG.error(
					"inside publishReplyEvents method of class EventHandler  got exception while send message to queueName={} and exception ",
					queueName, e);
			JmsMessages jmsMessage = jmsRepository.findByCorrelationId(correlationId);
			if (!ObjectUtils.isEmpty(jmsMessage)) {
				jmsMessage.setStatusDetail(e.getLocalizedMessage());
				jmsMessage.setStatus(String.valueOf(JMSConstants.REPLY_FAILED));
	...
	public void publishJMSEventWithoutReply(Object event, String queueName) {
		
		LOG.info("inside publishJMSEventWithoutReply method of class JmsEvents");
		try{
